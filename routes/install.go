...
import (
	...
	"gitote.com/gitote/gitote/pkg/cron"
	...
	"gitote.com/gitote/gitote/pkg/ssh"
	"gitote.com/gitote/gitote/pkg/template/highlight"
...
func InstallPost(c *context.Context, f form.Install) {
	...
	if err := models.NewTestEngine(x); err != nil {
		if strings.Contains(err.Error(), `Unknown database type: sqlite3`) {
			c.FormErr("DbType")
			c.RenderWithErr(c.Tr("install.sqlite3_not_available", "https://gitote.com/docs/installation/install_from_binary.html"), INSTALL, &f)
		} else {
			c.FormErr("DbSetting")
