...
The backup version must lower or equal to current Gitote version. You can also import
...
func runRestore(c *cli.Context) error {
	...
	backupVersion := metadata.Section("").Key("GITOTE_VERSION").MustString("999.0")
	if version.Compare(setting.AppVer, backupVersion, "<") {
		log.Fatal(0, "Current Gitote version is lower than backup version: %s < %s", setting.AppVer, backupVersion)
	}
