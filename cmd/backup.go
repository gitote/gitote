...
which can be used for migrating Gitote to another server. The output format is meant to be
portable among all supported database engines.`,
	...
	Flags: []cli.Flag{
		...
		stringFlag("archive-name", fmt.Sprintf("gitote-backup-%s.zip", time.Now().Format("20060102150405")), "Name of backup archive"),
...
const _ARCHIVE_ROOT_DIR = "gitote-backup"
...
func runBackup(c *cli.Context) error {
	...
	rootDir, err := ioutil.TempDir(tmpDir, "gitote-backup-")
	if err != nil {
	...
	metadata.Section("").Key("GITOTE_VERSION").SetValue(setting.AppVer)
