...
import (
	...
	"gitote.com/gitote/gitote/pkg/httplib"
...
func prepareHookTasks(e Engine, repo *Repository, event HookEventType, p api.Payloader, webhooks []*Webhook) (err error) {
	...
	for _, w := range webhooks {
		...
		// Use separate objects so modifcations won't be made on payload on non-Gitote type hooks.
...
func (t *HookTask) deliver() {
	...
	req := httplib.Post(t.URL).SetTimeout(timeout, timeout).
		...
		Header("X-Gitote-Delivery", t.UUID).
		Header("X-Gitote-Signature", t.Signature).
		Header("X-Gitote-Event", string(t.EventType)).
