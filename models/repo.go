...
import (
	...
	"gitote.com/gitote/gitote/pkg/sync"
)
...
func NewRepoContext() {
	...
	for configKey, defaultValue := range map[string]string{"user.name": "Gitote", "user.email": "gitote@fake.local"} {
...
func (repo *Repository) APIFormat(permission *api.Permission, user ...*User) *api.Repository {
	...
	apiRepo := &api.Repository{
		...
		// Reserved for go-gitote-client change
...
func initRepository(e Engine, repoPath string, doer *User, repo *Repository, opts CreateRepoOptions) (err error) {
	...
	tmpDir := filepath.Join(os.TempDir(), "gitote-"+repo.Name+"-"+com.ToStr(time.Now().Nanosecond()))
